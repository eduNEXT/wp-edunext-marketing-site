...
window.ENEXT_SRV = window.ENEXT_SRV || {};
window.ENEXT_SC = window.ENEXT_SC || {};

ENEXT.OPENEDX_DOMAIN = ENEXT_SRV.lms_base_url || ENEXT_SC.lms_base_url;
ENEXT.LOGIN_NEXT = ENEXT_SRV.lms_wp_return_path;
ENEXT.LOGIN_PATH = ENEXT_SRV.lms_login_path;
ENEXT.RUN_FUNCTIONS = [].concat(ENEXT_SRV.run_functions).concat(ENEXT_SC.run_shortcode);
...
ENEXT.prefillVisibleFields = function() {
    ENEXT.callApiAtOpenEdx('accounts').success(function(dataArray) {
        ...
        // Flatten the extended profile data and add it to the original profile data.
        data['extended_profile'].map(( ({field_name, field_value}) => data[field_name] = field_value));
...
ENEXT.replaceShortcodeFields = function() {
    ENEXT.callApiAtOpenEdx('accounts').success(function(dataArray) {
        var data = dataArray[0];
        jQuery('.js-order-info-sc').each(function(index, item){
            var $item = jQuery(item);
            if ($item.data('field') in data) {
                $item.html(data[$item.data('field')]);
            }
        });
    });
}
...
for (var i = 0; i < ENEXT.RUN_FUNCTIONS.length; i++) {
    if (ENEXT.RUN_FUNCTIONS[i] in ENEXT) {
