...
jQuery(document).ready(function ($) {
    ...
    ENEXT.setLabel = function (logged_in, menu_item) {

        menu_item.each(function () {
            let labels = $(this).text().split('/');
            $(this).text( logged_in ? labels[1] : labels[0]);
        });
    }
    ...
    ENEXT.renderMenu = function () {
        ...
        if (logged_in) {
            $(".login_or_menu_openedx > a:first-child").text('__username__');
            $(".login_or_menu_openedx > a:first-child").hide();
            $(".profile_openedx > a:first-child").attr("href", '__learner_profile__');
            $(".account_openedx > a:first-child").attr("href", '__account_settings__');
            $(".signout_openedx > a:first-child").attr("href", '__logout__');
            ENEXT.setLabel(logged_in, $(".login_or_dash_openedx > a:first-child"));
            $(".login_or_dash_openedx > a:first-child").attr("href",ENEXT_SRV_CS.dashboard_url);
            $(".menu_openedx > a:first-child").text('__username__');
            $(".menu_openedx > a:first-child").hide();
            $(".login_openedx").hide();
            $(".register_openedx").hide();
            $(".resume_openedx > a:first-child").attr("href",'__resume_block__');
            $(".dashboard_openedx > a:first-child").attr("href",ENEXT_SRV_CS.dashboard_url);
            ENEXT.renderLinks();
        } else {
            ENEXT.setLabel(logged_in, $(".login_or_menu_openedx > a:first-child"));
            $(".login_or_menu_openedx > a:first-child").attr("href",ENEXT_SRV_CS.login_url);
            $(".dashboard_openedx").hide();
            $(".profile_openedx").hide();
            $(".account_openedx").hide();
            $(".signout_openedx").hide();
            ENEXT.setLabel(logged_in, $(".login_or_dash_openedx > a:first-child"));
            $(".login_or_dash_openedx > a:first-child").attr("href",ENEXT_SRV_CS.login_url);
            $(".menu_openedx").hide();
            $(".resume_openedx").hide();
        }
    ...
    ENEXT.renderLinks = function () {
        ...
        $(".open-edx-link").each(function () {
            ...
            if (replace_name) {
                let new_name = replace_name[1];
                item.text = user_data[new_name];
                $(item).show()
            }

            if (replace_url) {
                let new_url = replace_url[1];
