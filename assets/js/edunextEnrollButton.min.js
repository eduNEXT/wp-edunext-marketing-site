...
ENEXT.registration = (function($){
  ...
  var isCourseRegistrationOpen = function (courseData) {
    ...
    var renderButtons = function(courseData, enrollmentData, $anchor) {
        var amIEnrolled = amIEnrolledIn(enrollmentData, courseData.course_id);      var hasCourseStarted = (new Date(Date.parse(courseData.course_start))) <= Date.now();
      ...
      if (amIEnrolled) {
        if (hasCourseStarted) {
          renderActionBtn('go_to_course', $anchor, OPENEDX_DOMAIN + '/courses/' + courseID + '/info');
        } else {
          renderActionBtn('course_has_not_started', $anchor, OPENEDX_DOMAIN + '/dashboard');
        }
      } else if (isInvitationOnly) {
        renderActionBtn('invitation_only', $anchor);
      } else if (isRegistrationOpen) {
        renderActionBtn('enroll_not_logged_in', $anchor, OPENEDX_DOMAIN + '/register?course_id=' + courseID + '&enrollment_action=enroll');
      } else {
