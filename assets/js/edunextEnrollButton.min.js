...
ENEXT.registration = (function($){
  ...
  var renderAction = function (action, $anchor, url) {
  ...
  var getCourseData = function(enrollmentApiLocation, courseID, enrollmentData, $anchor) {
    ...
    .done(function(data) {
      ...
      if (amIEnrolled) {
        if (hasCourseStarted) {
          renderAction('go_to_course', $anchor, OPENEDX_DOMAIN + "/courses/" + courseID + "/info");
        } else {
          renderAction('course_has_not_started', $anchor, OPENEDX_DOMAIN + "/dashboard");
        }
      } else if (isInvitationOnly) {
        renderAction('invitation_only', $anchor);
      } else if (isRegistrationOpen) {
        renderAction('enroll_not_logged_in', $anchor, OPENEDX_DOMAIN + "/register?course_id=" + courseID + "&enrollment_action=enroll");
      } else {
        renderAction('enrollment_closed', $anchor);
      }
    });
  }
