...
ENEXT.registration = (function($) {
    ...
    var renderActionBtn = function(action, $anchor, url) {
        ...
        url = url || '#';
        if (settings['hide_if'] === 'logged in' && amILoggedIn() || settings['hide_if'] === 'not logged in' && !amILoggedIn()) {
            $anchor.remove();
            return;
        }
        $anchor.show();
    ...
    var amILoggedIn = function() {
        return document.cookie.indexOf('edxloggedin=true') !== -1;
    }

    var amIEnrolledIn = function (enrollmentData, courseID) {
        return Boolean($.grep(enrollmentData, function(row) {
    ...
    var renderButtons = function(courseData, enrollmentData, $anchor) {
        ...
        if (amIEnrolled) {
            ...
            renderActionBtn('enroll', $anchor, OPENEDX_DOMAIN + userEnrollmentUrl);
        } else {
